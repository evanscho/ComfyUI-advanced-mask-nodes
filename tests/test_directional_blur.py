...
def test_radius_zero_unchanged(node):
    ...
    (result,) = node.directional_blur(mask, 180.0, 350.0, 0, True)
...
def test_preserve_interior(node):
    ...
    mask = torch.zeros(1, 100, 100)
    mask[:, 30:70, 30:70] = 1.0
    (result,) = node.directional_blur(mask, 180.0, 180.0, 10, True)
...
def test_2d_input(node):
    ...
    (result,) = node.directional_blur(mask_2d, 180.0, 350.0, 5, True)
    assert result.dim() == 3
